...
def process_excel(source_file, template_file, sheet_name):
    ...
    for img in source_sheet._images:
        if hasattr(img.anchor, '_from') and img.anchor._from:
            ...
            if img_row >= 3:  # 从第3行开始
                if img_row not in images_by_row:
                    images_by_row[img_row] = []
    ...
    for row_idx, row in enumerate(source_rows):
        ...
        # 处理B列(图片或文本)
        # WPS的DISPIMG公式无法被openpyxl处理,所以这里清空N列
        ...
        if source_row_num in images_by_row:
            for source_img in images_by_row[source_row_num]:
                try:
...
def main():
    ...
    for f in output_files:
        print(f"  - {f}")
    print("\n注意:")
    print("- 塑料球和陶瓷sheet使用WPS的DISPIMG公式,无法提取图片")
    print("- 外厂sheet有图片对象但位置不正确,可能需要手动调整")
    print("- 所有文本数据已按规则填充")
    print("="*50)
